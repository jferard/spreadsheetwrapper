...
public enum WrapperColor {
	...
	public static/*@Nullable*/WrapperColor getColorFromString(String color) {
		WrapperColor wrapperColor = null;
		if (color.startsWith("#")) {
			int minDistance = 1024;
			for (WrapperColor otherWrapperColor : WrapperColor.values()) {
				String otherColorAsHex = otherWrapperColor.toHex();
				int distance = WrapperColor.distance(color,
						otherColorAsHex);
			...
			try {
				wrapperColor = WrapperColor.valueOf(color);
	...
	private static int distance(String colorAsHex, String otherColorAsHex) {
		int distance = 0;
		for (int i = 1; i < 7; i += 2) {
