 ...
 *
 */
public abstract class AbstractSpreadsheetReaderTest { // NOPMD by Julien on 27/11/15 20:34
	/** content of cell A1 */
	...
	/** content of col C (= 3) */
	private static final List<String> COL_C_CONTENTS = Arrays
			.asList("ETIQUETTE,C,254",
					...
					"Ancien Hôtel de Belleval", "Hôtel de Joubert",
					"Hôtel d’Uston",
					"Ancienne maison ou couvent de la Miséricorde",
					"Hôtel de Bernard Duffau et ancien grand séminaire",
					"Hôtel Périer", "Hôtel de Boussugues",
					"Hôtel Pas de Beaulieu", "Hôtel de Castries", "Immeuble",
					...
					"Château de la Mogère", "Château et Parc de la Piscine",
					...
					"Hôtel de Roquemaure", "Hôtel d’Avèze", "Hôtel de Beaulac",
					"Hôtel Deydé", "Hôpital Général", "Hôtel de Saint-Félix",
					"Hôtel Lecourt", "Château d’Ô", "Collège des Ecossais");
	...
	/**
	 * read the content of cell A1 as boolean : must throw
	 * IllegalArgumentException
	 */
	@Test(expected = IllegalArgumentException.class)
	public final void testCellA1IsNotABoolean() {
		this.sheetReader.getBoolean(0, 0);
		Assert.fail();
	}

	/**
	 * read the content of cell A1 as date : must throw IllegalArgumentException
	 */
	@Test(expected = IllegalArgumentException.class)
	public final void testCellA1IsNotADate() {
		this.sheetReader.getDate(0, 0);
		Assert.fail();
	}

	/**
	 * read the content of cell A1 as double : must throw
	 * IllegalArgumentException
	 */
	@Test(expected = IllegalArgumentException.class)
	public final void testCellA1IsNotADouble() {
		this.sheetReader.getDouble(0, 0);
		Assert.fail();
	}

	/**
	 * read the content of cell A1 as formula : must throw
	 * IllegalArgumentException
	 */
	@Test(expected = IllegalArgumentException.class)
	public final void testCellA1IsNotAFormula() {
		Assert.assertEquals(null, this.sheetReader.getFormula(0, 0));
		Assert.fail();
	}

	/**
	 * read the content of cell A1 as integer : must throw
	 * IllegalArgumentException
	 */
	@Test(expected = IllegalArgumentException.class)
	public final void testCellA1IsNotAnInteger() {
		this.sheetReader.getInteger(0, 0);
		Assert.fail();
	}
	...
	public final void testCellA2AsADouble() {
		try {
			Assert.assertEquals(Double
					.valueOf(AbstractSpreadsheetReaderTest.CELL_A2_CONTENT),
	...
	public final void testCellA2IsNotAFormula() {
		Assert.assertEquals(null, this.sheetReader.getFormula(1, 0));
		Assert.fail();
	}

	/**
	 * read the content of cell A1 as text : must throw IllegalArgumentException
	 */
	@Test(expected = IllegalArgumentException.class)
	public final void testCellA2IsNotAText() {
		Assert.assertEquals(
				Integer.toString(AbstractSpreadsheetReaderTest.CELL_A2_CONTENT),
				this.sheetReader.getText(1, 0));
		Assert.fail();
	}
	...
	public final void testCellDateA2IsNotABoolean() {
		this.sheetReader.getBoolean(1, 0);
		Assert.fail();
	}
	...
	public final void testCellDateA2IsNotADate() {
		this.sheetReader.getDate(1, 0);
		Assert.fail();
	}

	/** read the content of col C */
	@Test
	...
	public final void testSheetNameAndDimensions() {
		...
		Assert.assertEquals(
				AbstractSpreadsheetReaderTest.COL_C_CONTENTS.size(),
				this.sheetReader.getRowCount());
		Assert.assertEquals(
				AbstractSpreadsheetReaderTest.ROW_2_CONTENTS.size(),
